...
(function () {
  const legacyIconMap = {
    'discovery-action': 'map-pin', 'hero-clock': 'clock-3', 'hero-home': 'home',
    'station-building': 'home', 'station-pin': 'map-pin', 'station-clock': 'clock-3',
    'station-arrow': 'arrow-right', 'station-empty': 'map-pin', 'station-more': 'chevron-down',
    'booking-back': 'arrow-left', 'booking-address': 'map-pin', 'booking-trust': 'check',
    'booking-location': 'map-pin', 'booking-calendar': 'calendar-days', 'booking-car': 'car-front',
    'booking-truck': 'truck', 'booking-camera': 'camera', 'booking-arrow': 'arrow-right',
    'booking-building': 'bookmark', 'booking-more': 'chevron-down', 'auth-check': 'check',
    'auth-mail': 'mail', 'header-chevron': 'chevron-down', 'cabinet-calendar': 'calendar-days',
    'cabinet-car': 'car-front', 'cabinet-empty': 'calendar-x', 'cabinet-close': 'x'
  };
  ...
  function replaceFirstTextWithIcon(element, iconName, glyphPattern) {
    ...
    if (firstText) firstText.textContent = firstText.textContent.replace(glyphPattern || /^\s*[⌖◷✓✉→←↓↑]\s*/, ' ');
  }
  ...
  function migrateLegacyGlyphs(root) {
    ...
    scope.querySelectorAll('.hero-benefit-icon').forEach(function (el) { if (!hasDeclaredIcon(el)) replaceLegacyIconElement(el, el.textContent.trim() === '⌂' ? legacyIconMap['hero-home'] : legacyIconMap['hero-clock']); });
